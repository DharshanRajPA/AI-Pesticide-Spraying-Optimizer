#!/usr/bin/env python3
"""
ASR and NLP pipeline for AgriSprayAI.
Implements Whisper for speech recognition and GPT-3 for structured symptom extraction.
"""
...
class GPT3SymptomExtractor:
    """GPT-3 based structured symptom extraction."""
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Extract symptoms from this farmer note: {text}"}
                ],
    ...
    def generate_rationale(self, symptoms: List[str], vision_confidence: float) -> str:
        ...
        try:
            ...
            response = openai.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an agricultural expert providing clear, actionable advice to farmers."},
...
class ASRNLPPipeline:
    """Complete ASR and NLP pipeline for AgriSprayAI."""
    ...
    def __init__(self, config_path: str = "configs/nlp_pipeline.yaml"):
        ...
        self.symptom_extractor = GPT3SymptomExtractor(
            model=self.config["gpt3"]["model"],
            temperature=self.config["gpt3"]["temperature"]
        )
...
def main():
    """Example usage of the ASR/NLP pipeline."""
    # Initialize pipeline
    pipeline = ASRNLPPipeline()
